...
`endif
	...
	// 			):(
	// 				(exu_in_bus_isBRANCH & ~exu_out_bus_dnpc_valid)?(exu_in_bus_snpc):(exu_out_bus_dnpc)
	// 			);
	assign dnpc=(wbu_out_bus_exception[4])?
			(
				(wbu_out_bus_exception[3:0]==`ysyx_26010011_EXCEPTION_MRET)?(csr_mepc):(csr_mtvec)
	...
	always @(*) begin
		if(flush_exception_valid) begin
			...
			// 		dnpc_valid = (idu_in_bus_pc != exu_out_bus_dnpc);
			// 	end else begin
			// 		dnpc_valid = 1;
			// 	end
...
`ifdef USE_VERILATOR
	...
	always @(posedge clock) begin
		...
		tb_FINAL_npc<=(wbu_in_bus_exception[4])?((wbu_out_bus_exception[3:0]==`ysyx_26010011_EXCEPTION_MRET)?csr_mepc:csr_mtvec):((wbu_in_bus_isJUMP | (wbu_in_bus_isBRANCH&wbu_in_bus_comp_result)) ? wbu_in_bus_lsu_result : wbu_in_bus_pc + 32'd4);//TODO MTVEC
